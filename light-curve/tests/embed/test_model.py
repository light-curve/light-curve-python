...
def test_band_groups_invalid_mapped_value():
    ...
    with pytest.raises(ValueError, match="Invalid values found"):
        lce.ATCAT.from_hf(output="last", band_groups={"g": 99})
...
def test_band_groups_single_dict_stored(atcat_lsst_band_groups):
    ...
    model = lce.ATCAT.from_hf(output="last", band_groups=atcat_lsst_band_groups)
    assert model.bands == atcat_lsst_band_groups
...
def test_band_groups_list_of_dicts_creates_two_mappings():
    ...
    model = lce.ATCAT.from_hf(output="last", band_groups=[{"g": 1, "r": 2}, {"i": 3, "z": 4}])
...
def test_band_groups_none_sets_valid_model_bands():
    ...
    model = lce.ATCAT.from_hf(output="last")
    assert model.defined_input_bands == lce.ATCAT.valid_model_bands
...
def atcat_last_model(atcat_lsst_band_groups):
    ...
    return lce.ATCAT.from_hf(output="last", reduction="beginning", band_groups=atcat_lsst_band_groups)
...
def test_list_band_groups_output_shape(atcat_lsst_band_groups):
    ...
    model = lce.ATCAT.from_hf(output="last", reduction="beginning", band_groups=band_groups)
    time, flux, flux_err, band = _make_lc(atcat_lsst_band_groups)
...
def test_integer_band_groups_none():
    ...
    model = lce.ATCAT.from_hf(output="last", reduction="beginning")
    n = 60
...
def test_single_band_no_bands_shape():
    ...
    model = lce.Astromer1.from_hf(output="mean", reduction="beginning")
...
def test_single_band_with_bands_shape():
    ...
    model = lce.Astromer1.from_hf(output="mean", reduction="beginning", bands=["g", "r"])
